        ...
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner order-info">
                    <h3>订单支付成功</h3>
                    <p class="payment-detail">我们将在您完成支付后的<span>24 小时内</span>发货</p>
                </div>
                <div class="box-inner payment-checkout-panel clear">
                    <span class="jianguo-blue-main-btn big-main-btn js-payment-order"  @click="payNowHandle()">
    	                <a>确定</a>
	    	            </span>
    	    	        <span class="is-pay">已完成</span>
                    <span class="prices"> 支付金额：   <em><span>¥ </span>{{submitOrder.totleFee}}.00</em>
                    </span>
                </div>
            </div>
            <div class="confirm-detail">
                <div class="info-title">订单编号</div>
                <p class="info-detail">{{submitOrder.orderId}}</p>
            </div>
            <div class="confirm-detail">
                <div class="info-title">收货信息</div>
                <p class="info-detail">姓名：{{submitOrder.receiveInfo.name}}</p>
                <p class="info-detail">联系电话：{{submitOrder.receiveInfo.mobilePhone}}</p>
                <p class="info-detail">详细地址：{{submitOrder.receiveInfo.street}}</p>
            </div>
            <div class="confirm-table-title clear">
                <span class="name fn-left">商品信息</span>
                <span class="subtotal fn-right">运费</span>
            ...
            <div class="confirm-goods-table">
                <div class="cart-items clear">
                    <div class="name fn-left hide-row">
                        <div class="name-cell">
                            <a href="javascript:;"  target="_blank">{{submitOrder.goodName}}</a><br>
                        </div>
                    </div>
                    <div class="subtotal fn-right">
                        <div class="subtotal-cell">¥ {{submitOrder.postage}}.00</div>
                    </div>
                    <div class="goods-num fn-right">1</div>
                    <div class="price fn-right">¥ {{submitOrder.price}}.00</div>
                </div>
            </div>
            <div class="order-discount-line">
                <p> 总计： <span>¥ {{submitOrder.totleFee}}.00</span> </p>
            </div>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        methods: {
            getSubmitOrder(){
                const submitOrder = JSON.parse(sessionStorage.getItem("submitOrder"))
                this.submitOrder = submitOrder
            },
            payNowHandle () {
                // alert('成功支付'+(tshis.orderInfo.price+this.orderInfo.freight)+'元')
